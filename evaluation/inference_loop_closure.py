...
from evaluate_kitti import compute_recall
...
def main_process(gpu, weights_path, args):
    ...
    for batch_idx, sample in enumerate(tqdm(ValidationLoader)):
        ...
        with torch.no_grad():
            ...
            batch_dict = model(model_in, None, metric_head=False, compute_rotation=False, compute_transl=False)
    ...
    fig = plt.figure()
    plt.plot(precision_ours_fp, recall_ours_fp, label='LCDNet (Protocol 1)', marker=marker, markevery=markevery)
    plt.xlim([0, 1])
    plt.ylim([0, 1])
    plt.xlabel("Recall [%]")
    ...
    #plt.show()
    fig.savefig(f'./result/po_30_08.pdf', bbox_inches='tight', pad_inches=0)
