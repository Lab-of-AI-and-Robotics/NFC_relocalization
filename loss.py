...
def sinkhorn_matches_loss(batch_dict, delta_pose, mode='pairs'):
    ...
    mask_ac = batch_dict['point_ac_mask']
    ...
    gt_dst_coords = gt_dst_coords[mask_ac].view(B,-1, 3)
    loss = (gt_dst_coords - sinkhorn_matches).norm(dim=2).mean()
    return loss

def sinkhorn_matches_ac_ne2po_ne_loss(batch_dict, delta_pose, mode='pairs'):
    sinkhorn_matches = batch_dict['sinkhorn_matches_ac_ne2po_ne']
    src_coords = batch_dict['point_coords']
    mask_ac_ne = ~batch_dict['point_ac_mask']
    src_coords = src_coords.clone().view(batch_dict['batch_size'], -1, 4)
    B, N_POINT, _ = src_coords.shape
    if mode == 'pairs':
        B = B // 2
    ...
    gt_dst_coords = torch.bmm(delta_pose.inverse(), src_coords.permute(0, 2, 1))
    gt_dst_coords = gt_dst_coords.permute(0, 2, 1)[:, :, :3]
    gt_dst_coords = gt_dst_coords[mask_ac_ne].view(B,-1, 3)
    loss = (gt_dst_coords - sinkhorn_matches).norm(dim=2).mean()
    return loss


def sinkhorn_matches_ac2po_ne_loss(batch_dict, delta_pose, mode='pairs'):
    sinkhorn_matches = batch_dict['sinkhorn_matches_ac2po_ne']
    src_coords = batch_dict['point_coords']
    #mask_ac = batch_dict['point_ac_mask']
    src_coords = src_coords.clone().view(batch_dict['batch_size'], -1, 4)
    B, N_POINT, _ = src_coords.shape
...
def sinkhorn_matches_ac_ne2po_loss(batch_dict, delta_pose, mode='pairs'):
    sinkhorn_matches = batch_dict['sinkhorn_matches_ac_ne2po']
    src_coords = batch_dict['point_coords']
    mask_ac_ne = ~batch_dict['point_ac_mask']
    src_coords = src_coords.clone().view(batch_dict['batch_size'], -1, 4)
    B, N_POINT, _ = src_coords.shape
...
def pose_negative_loss(batch_dict, delta_pose, mode='pairs'):
    src_coords = batch_dict['point_coords']
    src_coords = src_coords.clone().view(batch_dict['batch_size'], -1, 4)
    B, N_POINT, _ = src_coords.shape
    if mode == 'pairs':
    ...
    gt_dst_coords = gt_dst_coords.permute(0, 2, 1)[:, :, :3]
    # loss = (gt_dst_coords - sinkhorn_matches).norm(dim=2).mean()
    transformation = batch_dict['transformation_neg']
    pred_dst_coords = torch.bmm(transformation, src_coords.permute(0,2,1))
    pred_dst_coords = pred_dst_coords.permute(0, 2, 1)[:, :, :3]
    loss = torch.mean(torch.abs(pred_dst_coords - gt_dst_coords))
    # loss = (pred_dst_coords - gt_dst_coords).norm(dim=2).mean()
