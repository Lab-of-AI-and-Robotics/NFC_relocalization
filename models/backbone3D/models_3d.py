...
class NetVladCustom(nn.Module):
    def __init__(self, backbone, NV, feature_norm=False, fc_input_dim=256,
                 ...
                 sinkhorn_iter=5, use_svd=True, sinkhorn_type='unbalanced', is_training=True):
        super().__init__()
        self.backbone = backbone
        self.NV = NV
        self.feature_norm = feature_norm
    ...
    def forward(self, batch_dict, delta_pose, metric_head=True, compute_embeddings=True, compute_transl=True,
        ...
        if metric_head:
            ...
            # UOTHead
                ...
                batch_dict = self.pose_head(batch_dict, delta_pose, compute_transl, compute_rotation, mode=mode, is_training=self.is_training)
